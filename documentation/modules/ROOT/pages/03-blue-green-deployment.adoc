...
As mentioned before, we have a very simple application with two services. One frontal React app called *dstrategies-front* and one backend Quarkus app called *dstrategies-back*. We will use the front app to call several times the back app and see how the back version change during the deployment.
...
We will define an active or online service 'dstrategies-back-online' and a preview or offline service 'dstrategies-back-offline'. The idea is that the frontend will always use 'dstrategies-back-online' and 'dstrategies-back-offline' will only be used to test our service before triggering the rollout.
...
As the previous picture shows, it is required to generate a couple of services that will be used to redirect the traffic to the correct DStrategies App version. 

Please take a look at the files *./blue-green/dstrategies-back-service-online.yaml* and *./blue-green/dstrategies-back-service-offline.yaml* to understand the objects and get the names of these elements.
...
Please edit the file named *./blue-green/dstrategies-back-rollout.yaml* in oder to add the required **<SERVICE_OFFLINE>** and **<SERVICE_ONLINE>** service names:
...
Please edit the file named *./blue-green/dstrategies-back-analysis-template.yaml* to add the required **<USERNAME>** that helps to reference the user namespace in the healthcheck URL.
...
Finally, it is required to create a **Deployment** object to deploy the frontend application. This application helps us to test the blue-green rollout strategy from a web interface and see backend information.

To allow communication between the frontend and the backend, it is required to edit the file named *./blue-green/dstrategies-frontend-deployment.yaml* to add the required **<USERNAME>** and **<OCP_APPS_DOMAIN>**. You can check their values in the xref:01-setup.adoc#01-parameters[parameters chapter].
...
We are going to create the application *DStrategies* to apply the files modified previously to test Blue/Green deployment. Please edit the following yaml and set your own GitHub repository in the *reportURL*. Create a file called *application-dstrategies-blue-green.yaml*
...
We have deployed the *DStrategies* with ArgoCD. We can test the online and offline services to ensure that they are up and running.

To test the online service, it is required to extract the frontend URL:
...
Visit the frontend via your web browser, push - JUMP - button and ensure the following message is displaying on your screen:
...
We already deployed the *dstrategies-back* version V1.0.0, and we have a new dstrategies-back version V2.0.0 ready to be deployed. 

[#03-step1]
=== Step 1 - Deploy a new version
 
We will deploy a new version *V2.0.0*. To do so, we have to edit the files *./blue-green/dstrategies-back-rollout.yaml* and *./blue-green/dstrategies-back-configmap.yaml* to modify the value *V1.0.0* to *V2.0.0*:

../blue-green/dstrategies-back-rollout.yaml
[.console-input]
[source,input,yaml,subs="+macros,+attributes"]
----
...
../blue-green/dstrategies-back-configmap.yaml
[.console-input]
[source,input,yaml,subs="+macros,+attributes"]
----
...
To test the online service, you can use the frontend http://dstrategies-frontend-{user}-blue-green.apps.%CLUSTER%/[URL]
...
If this dependency is for example a production DB we will modify the Production environment.
...
Argo Rollouts provides this command to promote dstrategies-back:
[.console-input]
[source,input,subs="+macros,+attributes"]
----
...
To test the new online service, you can use the frontend http://dstrategies-frontend-{user}-blue-green.apps.%CLUSTER%/[URL]
...
We recommend making  the changes in git. We will revert the last commit
[.console-input]
[source,input,subs="+macros,+attributes"]
----
