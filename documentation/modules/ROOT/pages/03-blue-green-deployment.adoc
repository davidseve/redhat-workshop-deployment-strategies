...
Please edit a file named *./blue-green/dstrategies-back-analysis-template.yaml* in oder to add the required **<USERNAME>** that references the user namespace in the healthcheck url.
...
In order to allow communication between the frontend and the backend, it is required to edit the file named *./blue-green/dstrategies-frontend-deployment.yaml* in oder to add the required **<USERNAME>** and **<OCP_DOMAIN>**.

[source,yaml,subs="+macros,+attributes"]
----
        env:  
          - name: REACT_APP_BACK
            value: http://dstrategies-back-online-<USERNAME>-blue-green.<OCP_DOMAIN>/dstrategies-back
...
We will deploy a new version `V2.0.0`. To do it, we have to edit the file *./blue-green/dstrategies-back-rollout.yaml* and *./blue-green/dstrategies-back-configmap.yaml* in order to modify the value *v1.0.0* to *V2.0.0*:
