...
We have a very simple application that has two services. One frontal React app call `dstrategies-front` and one backend Quarkus app call `dstrategies-back`. We will use the front app to call several times the back app and see how the back version change during the deployment.
...
We will deploy a new version `V2.0.0`. To do it, we have to edit the file `helm/helm-umbrella-dstrategies/chart/values/blue-green/values-image-tag.yaml` under `dstrategies-back` set `tag` value to `V2.0.0`
