...
The following sections try to provide an overall vision about the application that will be used during this laboratory and the respective deployment strategies that will be study cases during the workshop.

== DStrategies App

DStrategies App is a microservice based application that is composed by a backend and a frontend microservice. The idea of this application is to be able to display the backend microservice caracteristics based on a request performed by the frontend.
