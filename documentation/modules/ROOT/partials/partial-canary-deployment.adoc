...
[source,input,subs="+macros,+attributes"]
----
curl "$(oc get routes dstrategies-back-online -n {user}-{namespace} --template='https://{{.spec.host}}')/dstrategies-back"
----
...
We will deploy a new version V2.0.0. To do it, we have to edit the file `helm/helm-umbrella-dstrategies/chart/values/{namespace}/values-image-tag.yaml` under `dstrategies-back` set `tag` value to `v.1.1.1`
